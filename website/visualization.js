function visualization(data)
{
    ...
    var mouseover = function(d) {
        Tooltip
        .style("opacity", 1)
        .html('<u>' + d.episodeName.split(/\s+/).slice(0,5).join(" ") + '</u>' + "<br>" + "by "+ d.author.split(/\s+/).slice(0,5).join(" ") + "<br>" + "Duration "+ d.duration.toString()+" mins");
        d3.select(this).style("fill", "red")
    }

    var mousemove = function(d) {
    return Tooltip.style("left", (d3.event.pageX-300) + "px").style("top", (d3.event.pageY-50)+ "px");
    }
    ...
    node.append("circle")
        ...
        .on("dblclick",function(d){moreInfo(d.episodeId,d.clusterId,"viz")})
        
    node.append("text")
        .attr("dy", ".3em")
        .style("text-anchor", "middle")
        .text(function(d) { return d.episodeName.substring(0,10); })
